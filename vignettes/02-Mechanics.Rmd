...
vignette: >
  %\VignetteIndexEntry{02-Mechanics}
...
A series of filters for ISBN, URL and year fields are applied to search for relevant extractable data from the remaining data. If the data is not available then it is set as NULL and will be skipped while writing the BibTeX file. 
...
The writer will read the bib chunks one at a time based on the metadata extracted and will write only those fields. The default entry type is a book, which should not have any problems with the web articles. 
